...
merge_inter=str(sys.argv[5]).upper()
...
for index, row in exp.iterrows():
    ...
    print('test')
    print(res.head())
    #res.name=row.index(res)
    res.columns=['dna_count','rna_count','n_obs_bc']
    res.index.name = 'name'
    
    res.reset_index(inplace=True)
    #res.insert(1,'name',row.index(res))
    ...
    def name(i):
        return (str(row['name']))
        #return (str(row['name'])+'_labeled')
    counts_filtered.to_csv('*-byInsert.tsv', index=False,name_function=name,sep='\t')
